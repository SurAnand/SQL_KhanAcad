/* Querying on a Public Dataset on Solar System Objects */
...
SELECT body, type_of_object, MAX(mean_radius) FROM solar_system_objects;
SELECT body, type_of_object, MIN(mean_radius) FROM solar_system_objects;
SELECT AVG(mean_radius) AS avg_radius_in_KM  FROM solar_system_objects;
SELECT body, MAX(mass) AS mass_tenToThe21KG FROM solar_system_objects WHERE mass;
SELECT body, MIN(mass) AS mass_tenToThe21KG FROM solar_system_objects WHERE mass;
SELECT body, mass AS mass_tenToThe21KG FROM solar_system_objects WHERE mass GROUP BY body HAVING mass > (SELECT mass from solar_system_objects WHERE body = "Earth");
SELECT body, mass AS mass_tenToThe21KG,
    CASE
        WHEN mass > (SELECT mass FROM solar_system_objects WHERE body = "Earth") THEN "Larger than Earth"
        WHEN mass = (SELECT mass FROM solar_system_objects WHERE body = "Earth")
        THEN "Earth Mass"
        ELSE "Smaller than Earth"
    END AS "Relative_to_Earth"
FROM solar_system_objects;

SELECT body FROM solar_system_objects WHERE mass_rel < 0.1 AND surface_gravity_rel < 0.1 AND volume_rel < 0.1;
SELECT COUNT(*) AS total_bodies FROM solar_system_objects;
SELECT COUNT(*) AS bodies_below_earth FROM solar_system_objects WHERE mass_rel < 0.1 AND surface_gravity_rel < 0.1 AND volume_rel < 0.1;